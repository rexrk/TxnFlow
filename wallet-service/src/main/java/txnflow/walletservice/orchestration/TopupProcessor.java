...
import txnflow.walletservice.wallet.dto.request.WalletCreditEvent;
...
public class TopupProcessor {
    ...
    @Transactional
//    @KafkaListener(topics = "topup-completed", groupId = "wallet-service")
    public void processTopup(WalletCreditEvent event) {
        ...
        log.info(
                "Processing topup from Kafka. userId={} amount={} ledgerId={}",
                userId,
                amount,
                ledgerId
        );
        ...
        boolean alreadyProcessed =
                walletTransactionRepository.existsByLedgerId(ledgerId);
        ...
        Wallet userWallet = walletRepository.findByUserIdForUpdate(userId)
                .orElseThrow(() -> {
                    log.warn(
                            "Topup failed: wallet not found. userId={}",
                            userId
                    );
        ...
        BigDecimal balanceAfter =
                userWallet.getBalance().add(amount);
        ...
        try {
            ...
            // DB-level idempotency safety net
            log.warn(
